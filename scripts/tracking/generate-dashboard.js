...
| Overall | ${config.sources[0].metrics.completion}% | ${config.sources[1].metrics.overall}% | ${config.sources[2].metrics.total_issues} issues | ${
  Math.abs(config.sources[0].metrics.completion - config.sources[1].metrics.overall) <= 5 ? '✅ Aligned' : '⚠️ Mismatch'
} |
...
|------|---------|----------------|--------|-----------------|`;
  ...
  for (const [taskId, evidence] of Object.entries(config.evidence_validation.results)) {
    ...
    const scoreText = evidence.evidence_score ? `${scoreColor} ${evidence.evidence_score}%` : '⏳ Pending';
...
['Assignment', 'Attendance', 'Grade Management'].forEach((system, index) => {
  ...
  const combinedStatus = progressScore >= 85 && (evidence?.verified || evidenceScore >= 70) ? '✅ READY' : '🔄 IN PROGRESS';
...
console.log(`   📊 Includes evidence validation: ${config.evidence_validation?.enabled ? 'YES' : 'NO'}`);
console.log(`   🔍 Last evidence check: ${config.evidence_validation?.last_validation ? new Date(config.evidence_validation.last_validation).toLocaleString() : 'Never'}`);