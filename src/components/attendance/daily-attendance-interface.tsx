...
export function DailyAttendanceInterface({ date }: DailyAttendanceInterfaceProps) {
  ...
  const getStatusIcon = (status: AttendanceStatus['status']) => {
    switch (status) {
      ...
      case 'excused': return <FileText className="h-4 w-4" />;
        ...
        <CardContent>
          <div className="space-y-4">
            {filteredStudents.map((student) => {
              ...
              return (
                <div key={student.id} className={`p-4 border rounded-lg ${isSelected ? 'border-blue-500 bg-blue-50' : ''}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      {student.previousAttendance && (
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className={getStatusColor(student.previousAttendance.status)}>
                            {getStatusIcon(student.previousAttendance.status)}
                            <span className="ml-1">{getStatusLabel(student.previousAttendance.status)}</span>
                          </Badge>
                        </div>
                      )}
                    </div>
