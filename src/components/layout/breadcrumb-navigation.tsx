    ...
    <nav className={cn('flex items-center space-x-2', className)}>
      ...
      {showBack && viewport.isMobile && parentRoute && (
        <Link href={parentRoute.href}>
          <TouchButton variant="ghost" size="sm" className="h-8 w-8 p-1" touchTarget="comfortable">
            ...
            <li key={item.href || index} className="flex items-center">
              ...
              {isCollapsed ? (
                    ...
                    {collapsedItems.map((collapsedItem) => (
                      <DropdownMenuItem key={collapsedItem.href} asChild>
                        <Link href={collapsedItem.href}>
                          {collapsedItem.icon && <collapsedItem.icon className="mr-2 h-4 w-4" />}
                          {collapsedItem.label}
                        </Link>
                      </DropdownMenuItem>
                    ))}
                ...
                <div className="flex items-center">
                  {isLast ? (
                    <span className="max-w-[150px] truncate font-medium text-gray-900 md:max-w-[200px] dark:text-gray-100">
                    ...
                    <Link
                      href={item.href}
                      className="max-w-[120px] truncate text-gray-500 transition-colors hover:text-gray-700 md:max-w-[150px] dark:text-gray-400 dark:hover:text-gray-200"
                    >
                      {item.icon && <item.icon className="mr-1 inline h-4 w-4" />}
                      {item.label}
                    </Link>
