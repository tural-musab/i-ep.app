...
// Tenant schema için özel type
export type TenantDatabase = Database['tenant_schema'];
...
export function getClientSupabaseClient(tenantId?: string) {
  ...
  const supabase = createClient<TenantDatabase>(supabaseUrl, supabaseAnonKey, {
    global: {
      headers: tenantId ? {
...
export function getTenantSupabaseClient(tenantId: string, req?: NextRequest) {  
  ...
  // Tenant schema type'ı ile client oluştur
  return createClient<TenantDatabase>(supabaseUrl, supabaseAnonKey, options);
}
