...
export class EnvironmentManager {
  ...
  private loadConfiguration(): EnvironmentConfig {
    ...
    return {
      ...
      database: {
        ...
        maxConnections: this.getNumberValue("DATABASE_MAX_CONNECTIONS", env === "production" ? 20 : 10),
      ...
      limits: {
        maxFileSize: this.getNumberValue("MAX_FILE_SIZE", env === "production" ? 10 * 1024 * 1024 : 50 * 1024 * 1024),
        maxRequestSize: this.getNumberValue("MAX_REQUEST_SIZE", env === "production" ? 5 * 1024 * 1024 : 10 * 1024 * 1024),
        maxConcurrentRequests: this.getNumberValue("MAX_CONCURRENT_REQUESTS", env === "production" ? 100 : 50),
        rateLimitWindow: this.getNumberValue("RATE_LIMIT_WINDOW", 60 * 1000),
        rateLimitMaxRequests: this.getNumberValue("RATE_LIMIT_MAX_REQUESTS", env === "production" ? 100 : 1000)
      },
      external: {
        supabase: {
  ...
  private validateConfiguration(): void {
    ...
    if (this.environment === "production") {
      ...
      if (this.config.external.iyzico.enabled && (!this.config.external.iyzico.apiKey || !this.config.external.iyzico.secretKey)) {
