...
export const SCHEMAS: { [key: string]: OpenAPIV3.SchemaObject } = {
  ...
  ReportRequest: {
    ...
    properties: {
      type: {
        type: 'string',
        enum: ['student_progress', 'grade_summary', 'attendance_report', 'class_performance'],
      },
      title: { type: 'string', minLength: 1, maxLength: 255 },
      parameters: {
        type: 'object',
...
export async function getOpenAPISpec(): Promise<OpenAPIV3.Document> {
  ...
  for (const endpoint of API_ENDPOINTS) {
    ...
    spec.paths[endpoint.path][endpoint.method.toLowerCase() as keyof OpenAPIV3.PathItemObject] =
      operation;
  }
