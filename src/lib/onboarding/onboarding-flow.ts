...
export type OnboardingStep = 'welcome' | 'school_setup' | 'user_profile' | 'class_setup' | 'integration_setup' | 'completion'
...
export class OnboardingFlowManager {
   ...
   */
  async completeStep(step: OnboardingStep, stepData: Record<string, any> = {}): Promise<OnboardingProgress> {
    try {
      ...
      const template = this.getOnboardingTemplate(progress.user_preferences.role_specific_guidance ? 'admin' : 'teacher')
      ...
      const { data, error } = await this.supabase
        ...
        .update({
          ...
          actual_completion_time: nextStep === 'completion' ? 
            Math.round((Date.now() - new Date(progress.started_at).getTime()) / 1000) : null
        })
        .eq('id', progress.id)
        .select()
        .single()
  ...
  async getOnboardingMetrics(): Promise<OnboardingMetrics> {
    try {
      ...
      const averageCompletionTime = completedUsers > 0 ?
        progressRecords
      ...
      const steps: OnboardingStep[] = ['welcome', 'school_setup', 'user_profile', 'class_setup', 'integration_setup', 'completion']
      
      steps.forEach(step => {
        const usersWhoReachedStep = progressRecords?.filter(p => 
          p.completed_steps.includes(step) || p.current_step === step
        ).length || 0
        stepCompletionRates[step] = totalUsers > 0 ? (usersWhoReachedStep / totalUsers) * 100 : 0
      })

      // Find most common exit step
      const exitSteps = progressRecords?.filter(p => !p.completed_at).map(p => p.current_step) || []
      const mostCommonExitStep = exitSteps.reduce((acc, step) => {
        acc[step] = (acc[step] || 0) + 1
        return acc
      }, {} as Record<string, number>)

      const mostCommonExit = Object.entries(mostCommonExitStep).reduce((a, b) => 
      ...
      return {
        ...
        support_requests_during_onboarding: events?.filter(e => e.event_type === 'support_requested').length || 0
      }
    } catch (error) {
   ...
   */
  async updatePreferences(preferences: Partial<OnboardingProgress['user_preferences']>): Promise<void> {
    try {
      const progress = await this.getOnboardingProgress()
      if (!progress) return
    ...
    const baseTemplate: OnboardingTemplate = {
      ...
      steps: [
          ...
          components: [
            {
              ...
              config: {
                ...
                slides: [
                  ...
                  { title: 'Özellikler', content: 'Not yönetimi, devamsızlık takibi, veli iletişimi' },
          ...
          components: [
            {
              ...
              config: {
                fields: [
                  ...
                  { name: 'notifications', type: 'checkbox-group', options: ['email', 'sms', 'push'] },
        ...
        },
        {
          id: 'completion',
          title: 'Tamamlandı',
          description: 'Onboarding süreci tamamlandı! Artık İ-EP.APP\'i kullanmaya başlayabilirsiniz.',
          estimated_time: 60,
          required: true,
          role_specific: ['admin', 'teacher', 'student', 'parent'],
  ...
  private getNextStep(completedSteps: OnboardingStep[], template: OnboardingTemplate): OnboardingStep {
    const orderedSteps: OnboardingStep[] = ['welcome', 'school_setup', 'user_profile', 'class_setup', 'integration_setup', 'completion']
  ...
  private validateStepData(stepConfig: OnboardingStepConfig, stepData: Record<string, any>): {
    valid: boolean
    errors: string[]
  } {
    const errors: string[] = []
...
// Utility functions
export function calculateOnboardingProgress(completed: OnboardingStep[], total: number = 6): number {
...
export function estimateTimeRemaining(currentStep: OnboardingStep, template: OnboardingTemplate): number {
