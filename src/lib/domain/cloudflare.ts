...
export class CloudflareDomainManager implements DomainProvider {
  ...
  async setupCustomDomain(
    ...
    try {
      ...
      const response = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${this.config.zoneId}/custom_hostnames`,
        {
          method: 'POST',
          headers: {
  ...
  async removeSubdomain(tenantId: string, subdomain: string): Promise<boolean> {
    try {
      ...
      const listResponse = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${this.config.zoneId}/dns_records?name=${subdomain}.${this.config.baseDomain}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${this.config.apiToken}`,
  ...
  async verifyCustomDomain(domain: string): Promise<DomainVerificationStatus> {
    try {
      ...
      const response = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${this.config.zoneId}/custom_hostnames?hostname=${domain}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${this.config.apiToken}`,
