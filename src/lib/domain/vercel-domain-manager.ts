...
export class VercelDomainManager implements DomainProvider {
  ...
  async createSubdomain(tenant: { id: string; subdomain: string }): Promise<boolean> {
    try {
      console.log(`Vercel API: Subdomain oluşturuluyor: ${tenant.subdomain}.${this.config.baseDomain}`);
      
      const domain = `${tenant.subdomain}.${this.config.baseDomain}`;
      
      // Vercel API'ye domain ekleme isteği
      const response = await fetch(`https://api.vercel.com/v10/projects/${this.config.projectId}/domains`, {
        method: 'POST',
        headers: {
  ...
  async verifyCustomDomain(domain: string): Promise<DomainVerificationStatus> {
    try {
      ...
      const response = await fetch(`https://api.vercel.com/v9/projects/${this.config.projectId}/domains/${domain}/status`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${this.config.apiToken}`,
          'Content-Type': 'application/json'
      ...
      console.log(`Vercel API: Domain durumu: ${domain} - Doğrulandı: ${verified}, SSL: ${sslActive}, Durum: ${status}`);
  ...
  async removeSubdomain(tenantId: string, subdomain: string): Promise<boolean> {
    try {
      ...
      const response = await fetch(`https://api.vercel.com/v9/projects/${this.config.projectId}/domains/${domain}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${this.config.apiToken}`,
          'Content-Type': 'application/json'
