...
import { verifyDomainConfiguration, checkDNSConfiguration, checkSSLStatus } from './domain-verification';
...
export class DomainService {
   ...
   */
  async addCustomDomain(tenantId: string, domain: string, isPrimary = false): Promise<{
  ...
  async setPrimaryDomain(domainId: string, tenantId: string): Promise<boolean> {
    ...
    if (domain.tenant_id !== tenantId) {
      throw new TenantDomainError(
        domain.domain,
        'Bu domain belirtilen tenant\'a ait değil.'
      );
    }
   ...
   */
  private async updateDomainVerification(domainId: string, isVerified: boolean, verifiedAt?: string): Promise<void> {
    const supabase = createServerSupabaseClient();
