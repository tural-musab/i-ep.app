...
export async function deleteUserData(
  ...
  try {
    logger.info(
      `GDPR veri silme işlemi başlatıldı. Kullanıcı: ${userId}, Tenant: ${tenantId}, Tip: ${options.type}`
    );
 ...
 */
async function softDeleteUserData(userId: string, tenantId: string, retentionPeriodDays = 30) {
  const deletedTables: string[] = [];
  let totalDeletedRecords = 0;
  ...
  try {
    ...
    if (userUpdateError) {
      throw new Error(
        `Kullanıcı kaydı soft delete işaretlenirken hata: ${userUpdateError.message}`
      );
    }
...
async function logDeletionRequest(
  ...
  try {
    const { error } = await supabaseAdmin.from('gdpr_deletion_requests').insert({
      id: requestId,
      user_id: userId,
      tenant_id: tenantId,
...
async function sendUserDeletionNotification(
  ...
  try {
    ...
    logger.info(
      `Kullanıcı ${userId} için ${deletionType} silme bildirimi gönderildi. Email: ${userData.email}`
    );
  } catch (error) {
...
async function sendAdminDeletionNotification(
  ...
  try {
    ...
    // Bu örnekte sadece log kaydı oluşturuyoruz
    logger.info(
      `Admin ${adminData.email} için kullanıcı ${userId} silme bildirimi gönderildi. Tip: ${deletionType}`
    );
  } catch (error) {
  ...
  try {
    ...
    for (const deletion of pendingDeletions || []) {
      try {
        ...
        logger.info(
          `Kullanıcı ${deletion.user_id} için zamanlanmış kalıcı silme işlemi tamamlandı`
        );
      } catch (deletionError) {
