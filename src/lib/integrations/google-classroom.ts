...
export class GoogleClassroomIntegration {
  ...
  async getAssignments(courseId: string): Promise<GoogleClassroomAssignment[]> {
    try {
      ...
      return (
        response.data.courseWork?.map((assignment: any) => ({
          id: assignment.id,
  ...
  async getSubmissions(
    ...
    try {
      ...
      return (
        response.data.studentSubmissions?.map((submission: any) => ({
          id: submission.id,
...
export function createGoogleClassroomIntegration(): GoogleClassroomIntegration {
  ...
  return new GoogleClassroomIntegration({
    ...
    redirectUri:
      process.env.GOOGLE_REDIRECT_URI || `${config.external.supabase.url}/auth/callback/google`,
    scopes: [
...
// Utility functions
export function formatGoogleClassroomDate(date: {
  year: number;
  month: number;
  day: number;
}): string {
