...
export class PredictiveAnalytics {
   ...
   */
  async forecastResourceDemand(resourceTypes: string[] = ['classroom', 'teacher', 'textbook']): Promise<ResourceDemandForecast[]> {
    try {
  ...
  private async calculateStudentRisk(studentId: string, analytics: any): Promise<StudentRiskPrediction> {
  ...
  private async calculateGradePrediction(studentId: string, assignment: any): Promise<GradePrediction> {
    ...
    const factors = [
      {
        ...
        current_value: pastGrades?.length ? 
          pastGrades.reduce((sum, g) => sum + (g.points / g.max_points), 0) / pastGrades.length : 0.75
      },
      {
    ...
    const predictedGrade = factors.reduce((sum, factor) => {
  ...
  private async calculateAttendancePrediction(studentId: string, date: Date): Promise<AttendancePrediction> {
    ...
    const weekdayAttendance = pastAttendance?.filter(a => {
      const aDate = new Date(a.date)
      return aDate.getDay() === dayOfWeek
    }) || []

    const weekdayRate = weekdayAttendance.length > 0 ?
      weekdayAttendance.filter(a => a.status === 'present').length / weekdayAttendance.length : 0.9
  ...
  private async calculateEnrollmentForecast(year: number, historicalData: any[]): Promise<EnrollmentForecast> {
  ...
  private async calculateResourceDemandForecast(resourceType: string): Promise<ResourceDemandForecast> {
    ...
    const avgUsage = currentUsage?.length ?
      currentUsage.reduce((sum, u) => sum + u.usage, 0) / currentUsage.length : 0
  ...
  private getRiskFactorDescription(factor: string, value: number): string {
    const descriptions: Record<string, string> = {
      ...
      'Assignment Completion': value > 50 ? 'Missing assignments' : 'Completing assignments on time',
