...
export async function restoreFullTenant(
  ...
  try {
    ...
    // Sıkıştırılmışsa aç
    if ((backup.s3_path && backup.s3_path.endsWith('.gz')) || 
    ...
    if (options.validateAfterRestore) {
      ...
      if (!validationSuccess) {
        throw new Error('Geri yükleme sonrası doğrulama başarısız oldu. Veri tutarsızlığı olabilir.');
      }
    ...
    if (options.notifyAdmin) {
      await sendRestoreNotification(tenantId, false, error instanceof Error ? error.message : String(error));
    }
...
export async function restorePartialTenant(
  ...
  try {
    logger.info(`Tenant ${tenantId} için kısmi geri yükleme başlatılıyor. Hedef tablolar: ${targetTables.join(', ')}`);
    ...
    for (const tableName of targetTables) {
      ...
      await executeSQL(`CREATE TABLE tenant_${tenantId}.${tableName} (LIKE ${tempSchema}.${tableName} INCLUDING ALL)`);
      
      // Veriyi aktar
      await executeSQL(`INSERT INTO tenant_${tenantId}.${tableName} SELECT * FROM ${tempSchema}.${tableName}`);
...
async function downloadFromS3(s3Path: string): Promise<Buffer> {
  try {
    ...
    throw new Error(`S3'ten indirme başarısız: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function decryptData(data: Buffer, key: string): Buffer {
  try {
    ...
    throw new Error(`Şifre çözme başarısız: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function executeSQL(sql: string): Promise<void> {
  try {
    ...
    if (stderr && !stderr.includes('CREATE') && !stderr.includes('INSERT') && !stderr.includes('ALTER')) {
 ...
 */
async function validateTenantTables(tenantId: string, tables: string[]): Promise<ValidationResult[]> {
