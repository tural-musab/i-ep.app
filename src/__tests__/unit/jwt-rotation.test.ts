...
describe('JWT Secret Rotation', () => {
  ...
  describe('Error Handling', () => {
    it('should handle rotation errors gracefully', async () => {
      ...
      managerWithPrivate.logRotationEvent = jest.fn().mockRejectedValue(new Error('Audit service down'));
  ...
  describe('Security Features', () => {
    ...
    it('should mask secrets in logs', async () => {
      ...
      const secretLogCall = logCalls.find(call => 
        call[0] === 'JWT secrets should be updated in external key management service'
      );
