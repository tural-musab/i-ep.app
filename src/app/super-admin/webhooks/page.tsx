...
import { PlusCircle, Edit2, Trash2, CheckCircle, XCircle, Code, PlayCircle, Clock, RefreshCw } from 'lucide-react';
...
export default function WebhooksPage() {
  ...
  async function fetchWebhooks() {
    ...
    try {
      ...
      const enrichedWebhooks = await Promise.all((webhooks || []).map(async (webhook) => {
        let tenantName = null;
        ...
        <DialogContent className="sm:max-w-[550px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Belirli bir tenant seçerseniz, yalnızca bu tenant ile ilgili olaylar için webhook tetiklenecektir.</p>
            </div>
        ...
        <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedWebhook.description || 'Açıklama yok'}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Bu değeri gelen isteklerin imzalarını doğrulamak için kullanın</p>
              </div>
              
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Abone Olunan Olaylar</Label>
                <div className="flex flex-wrap gap-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <div className="text-sm">
                    {format(new Date(selectedWebhook.created_at), 'dd MMM yyyy HH:mm', { locale: tr })}
                  </div>
                </div>
                
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Son Güncellenme</Label>
                  <div className="text-sm">
                    {format(new Date(selectedWebhook.updated_at), 'dd MMM yyyy HH:mm', { locale: tr })}
                  </div>
                </div>
                
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Başarılı İstek Sayısı</Label>
                  <div className="text-sm">
                    {selectedWebhook.success_count || 0}
                  </div>
                </div>
                
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Başarısız İstek Sayısı</Label>
                  <div className="text-sm">
                    {selectedWebhook.failure_count || 0}
                  </div>
                </div>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsViewDialogOpen(false)}>Kapat</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
          ...
          <DialogContent className="sm:max-w-[600px]">
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">Bu JSON verisi webhook endpointine gönderilecektir</p>
              </div>
              ...
              {testResponse && (
                <div className="space-y-2 border-t pt-4">
                  ...
                  {testResponse.error ? (
                    <div className="bg-red-50 text-red-700 p-3 rounded-md text-sm font-mono">
                      {testResponse.error}
                    </div>
                  ) : (
                    <div className="bg-muted p-3 rounded-md overflow-auto max-h-40">
                      <pre className="text-xs font-mono whitespace-pre-wrap">{testResponse.body}</pre>
                    </div>
                  )}
                </div>
              )}
            ...
            <DialogFooter>
              ...
              <Button variant="outline" onClick={() => setIsTestDialogOpen(false)}>İptal</Button>
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="py-4">
              <p className="text-sm text-destructive">
                Bu webhook silindiğinde, abonelikler de silinecek ve ilgili olaylar için bildirimler artık gönderilmeyecektir.
              </p>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsDeleteDialogOpen(false)}>İptal</Button>
              <Button variant="destructive" onClick={handleDeleteWebhook}>Sil</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
