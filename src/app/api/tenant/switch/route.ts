  ...
  try {
    ...
    if (!targetTenantId) {
      return NextResponse.json(
        { error: 'Geçerli bir tenant ID belirtilmelidir' },
        { status: 400 }
      );
    }
    ...
    if (!isSuperAdmin) {
      ...
      const parsedAllowedTenants = typeof allowedTenants === 'string'
        ? JSON.parse(allowedTenants)
        : (Array.isArray(allowedTenants) ? allowedTenants : []);
      
      const canAccessTenant = parsedAllowedTenants.includes(targetTenantId) ||
    ...
    if (!tenantData.is_active) {
      return NextResponse.json(
        { error: 'Bu tenant şu anda aktif değil' },
        { status: 403 }
      );
    }
    ...
    const primaryDomain = primaryDomainData?.domain || 
