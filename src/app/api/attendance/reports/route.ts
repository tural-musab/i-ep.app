...
async function generateSummaryReport(
  ...
  const summaryData = await attendanceRepo.getAttendanceReport(classId, startDate, endDate);
...
async function generateClassReport(
  ...
  const classRecords = await attendanceRepo.getAttendanceReport(classId, startDate, endDate);
...
async function generateDailyReport(
  ...
  const dailyRecords = await attendanceRepo.getAttendanceReport(classId, targetDate, targetDate);

  let classSummary;
  if (classId) {
    classSummary = await attendanceRepo.getClassAttendanceSummary(classId, targetDate);
  }
...
async function generateMonthlyReport(
  ...
  const monthlyRecords = await attendanceRepo.getAttendanceReport(classId, startDate, endDate);

  // Group by month
  const monthlyData = monthlyRecords.reduce(
    (acc, record) => {
