 ...
 */
export async function PUT(req: NextRequest, { params }: { params: Promise<{ domainId: string }> }) {
  try {
    ...
    if (!tenantId) {
      return NextResponse.json({ success: false, error: 'Tenant ID gereklidir' }, { status: 400 });
    }
